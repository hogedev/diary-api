...
async def get_entry(entry_id: int, service: EntryServiceDep) -> EntryResponse:
    return EntryResponse.model_validate(await service.get_by_id(entry_id))


@router.put("/{entry_id}", response_model=EntryResponse)
async def update_entry(entry_id: int, data: EntryUpdate, service: EntryServiceDep) -> EntryResponse:
    return EntryResponse.model_validate(await service.update(entry_id, data))


@router.delete("/{entry_id}", status_code=204)
async def delete_entry(entry_id: int, service: EntryServiceDep) -> None:
...
async def add_photos(
    ...
    return EntryResponse.model_validate(await service.add_photos(entry_id, photos))
